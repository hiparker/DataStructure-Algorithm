...
public class DepthFirstSearch {
    ...
    public static void main(String[] args) {
        ...
        // 深度优先遍历原始图
        System.out.println("输出：深搜遍历结果");
        dfsTraverseGraph(diGraph);
        System.out.println();
     ...
     * 深搜遍历原始图
     */
    public static void dfsTraverseGraph(DiGraph diGraph){
        if(null == diGraph){
            return;
        }
        ...
        firstStack.push(new int[]{0,0});
        Stack<int[]> indexStack = new Stack<>();

        System.out.print(diGraph.getPointName(0) + "  ");

        while (!firstStack.isEmpty() || !indexStack.isEmpty()){
            int[] pop = !firstStack.isEmpty()
                    ? firstStack.pop()
                    : indexStack.pop();
            ...
            for (int k = pop[1]; k < ints.length; k++) {
                if(ints[k] == 1 && flag[k] == 0){
                    ...
                    indexStack.push(new int[]{i,k+1});
                    // 去往命中行
                    firstStack.push(new int[]{k,0});
                    flag[k] = 1;
                    break;
                }
